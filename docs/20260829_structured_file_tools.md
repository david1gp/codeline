...
- [x] 8. Port integration tests before integration code: adapt registration cases from `deepseek-harness/packages/fs/tool-fs/src/index.ts` and `tool-fs/tests/tools.spec.ts` into `test/tools/runtime/fileToolRegistration.test.ts`, and update existing manifest/provider tests to specify `read`, `write`, and `edit` alongside `bash`.
