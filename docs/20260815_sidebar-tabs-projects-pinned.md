# Sidebar tabs, projects, and pinned sessions

## Goal

Redesign the workspace sidebar with full-width New Session and subtle New Project actions, icon tabs for recent, pinned, projects, and search, two-line session rows, collapsible project groups, folder-path project selection, and persisted pinned/project state for every session.

## Decisions

- A session stores a required project path reference and a pinned boolean; new sessions default to pinned.
...
- Recent and pinned are flat active-session lists. Projects group active sessions in collapsible native details. Search uses the existing server search and renders the same two-line rows.
...
- The pin toggle lives in the selected session header and updates immediately through an authenticated session API.

## Approach

- Extend session persistence, Zero schema/query data, creation, fixture seeding, and authenticated actions for pinned and project association.
...
- [x] 1. Add and test session pinned/project persistence, defaults/backfill, API toggle, Zero exposure, and creation validation.
...
- [x] 4. Add tested sidebar derivation for recent, pinned, project groups, search results, and two-line relative/absolute timestamps.
- [x] 5. Implement the sidebar action hierarchy, icon tabs, session rows, collapsible project groups, and selected-session pin toggle.
