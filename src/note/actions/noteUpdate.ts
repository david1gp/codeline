...
export async function noteUpdate(
  ...
  if (!parsed.success) return createResultError("noteUpdate", "The note update input is invalid.")
  ...
  if (parsed.output.projectId !== null) {
    ...
    if (!existing.success) return createResultError("noteUpdate", existing.errorMessage)
    if (existing.data === undefined) return createResultError("noteUpdate", "The note could not be found.")
  ...
  if (!projectPath.success) return createResultError("noteUpdate", projectPath.errorMessage)
