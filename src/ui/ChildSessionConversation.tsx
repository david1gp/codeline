...
export function ChildSessionConversation(props: { sessionId: string }) {
  const state = childSessionConversationStateCreate(() => props.sessionId)
    ...
    <section aria-label="Child conversation" class="grid gap-3">
      ...
      <ol class="m-0 grid list-none gap-1 p-0" aria-label="Child conversation activity">
        <For each={state.history.semanticSteps()}>
          {(step) => <SessionSemanticStepRow sessionId={props.sessionId} step={step} />}
        </For>
      </ol>
    </section>
  )
