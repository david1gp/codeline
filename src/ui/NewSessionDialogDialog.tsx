...
type NewSessionDialogDialogProps = {
...
/** New-session dialog shell with Corvu's supported initial focus target. */
export function NewSessionDialogDialog(p: NewSessionDialogDialogProps) {
  const texts = p.texts ?? corvuDialogTextDefault

  return (
    <Dialog initialFocusEl={p.initialFocusEl} open={p.open} onOpenChange={p.onOpenChange}>
      <Dialog.Trigger
        class={buttonCva2(
          p.variant,
          undefined,
