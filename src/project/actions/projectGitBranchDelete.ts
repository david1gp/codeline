...
type ProjectGitBranchDeleteOptions = {
  command?: ProjectGitCommand
}

export async function projectGitBranchDelete(
  rootDir: string,
  branch: string,
  options: ProjectGitBranchDeleteOptions = {},
): Promise<Result<void>> {
  const op = "projectGitBranchDelete"
  ...
  if (repository.data.branch === parsedBranch.output) {
    return createResultError(op, "The current Git branch cannot be deleted.")
  }

  try {
    const result = await repository.data.command(["branch", "-D", "--", parsedBranch.output])
    if (!result.success || result.data.exitCode !== 0) {
      return createResultError(op, "The Git branch could not be deleted.")
    }
  } catch (_error) {
    return createResultError(op, "The Git branch could not be deleted.")
  }
