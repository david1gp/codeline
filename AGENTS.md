...
- After changing `.env` Zero URLs, restart the managed `codeline-dev-zero-cache` unit. After schema or migration changes that leave a stale replica, run `./ops/dev/codeline-dev.sh clean`; it restarts the managed target and remigrates/reseeds automatically. Do not start a host `zero-cache-dev` or a second cache.
