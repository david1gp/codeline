...
test("normalizes TanStack and OpenAI-compatible usage fields", () => {
  expect(
    compactionTokenUsageResolve({ promptTokens: 100, completionTokens: 20, totalTokens: 120 }),
  ).toEqual({ inputTokens: 100, outputTokens: 20, totalTokens: 120 })
  expect(
    compactionTokenUsageResolve({ prompt_tokens: 200, completion_tokens: 30, total_tokens: 230 }),
  ).toEqual({ inputTokens: 200, outputTokens: 30, totalTokens: 230 })
