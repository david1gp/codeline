...
test("provider model selector renders non-selectable provider groups with selectable models beneath", async () => {
  ...
  expect(component).toContain("<option>Preparing conversation...</option>")
  expect(component).toContain("Model selection will be available when the conversation is ready.")
