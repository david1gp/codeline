...
test("the initial composer keeps execution and provider targets visible", async () => {
  const selectedSession = await Bun.file(new URL("../../src/session/ui/SelectedSession.tsx", import.meta.url)).text()
...
test("the setup panel reports organization machine availability and exposes one execution agent", async () => {
  const setupPanel = await Bun.file(new URL("../../src/session/ui/WorkspaceSetupPanel.tsx", import.meta.url)).text()
