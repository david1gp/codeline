...
test("Project files views stay view-only and receive injected state", async () => {
  const filesPage = await Bun.file(new URL("../../../src/project/ui/FilesPage.tsx", import.meta.url)).text()
  const browser = await Bun.file(new URL("../../../src/project/ui/ProjectBrowser.tsx", import.meta.url)).text()
  const gitPanel = await Bun.file(new URL("../../../src/project/ui/ProjectGitPanel.tsx", import.meta.url)).text()
...
test("Project files route composes production state outside the view", async () => {
  const routePage = await Bun.file(new URL("../../../src/project/ui/FilesRoutePage.tsx", import.meta.url)).text()
