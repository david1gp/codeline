...
import { sessionChatStateCacheCreate } from "../../../src/session/ui/sessionChatStateCacheCreate.js"
import type { SessionChatState } from "../../../src/session/ui/sessionChatStateCreate.js"
...
test("selected session state builds the chat accessor from the per-session cache", async () => {
  const view = await Bun.file(new URL("../../../src/session/ui/SelectedSession.tsx", import.meta.url)).text()
  const state = await Bun.file(new URL("../../../src/session/ui/selectedSessionStateCreate.ts", import.meta.url)).text()
