...
const { sessionPinToggleStateCreate } = await import("../../../src/session/ui/sessionPinToggleStateCreate.js")
...
test("selected-session pin state follows the source query after a successful toggle", async () => {
  const selectedSessionSource = await Bun.file(
    new URL("../../../src/session/ui/selectedSessionStateCreate.ts", import.meta.url),
  ).text()
