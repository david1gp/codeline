...
test("selected session exposes rename at its HTTP-backed title without navigation callbacks", async () => {
  const source = await Bun.file(new URL("../../../src/session/ui/SelectedSession.tsx", import.meta.url)).text()

  expect(source).toContain('import { SessionRenameControl } from "./SessionRenameControl.js"')
