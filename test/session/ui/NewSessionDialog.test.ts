...
const dialog = await Bun.file(new URL("../../../src/session/ui/NewSessionDialog.tsx", import.meta.url)).text()
const genericDialog = await Bun.file(
  new URL("../../../src/session/ui/NewSessionDialogDialog.tsx", import.meta.url),
).text()
const state = await Bun.file(new URL("../../../src/session/ui/newSessionDialogStateCreate.ts", import.meta.url)).text()
...
test("New Session opens the production project dialog through workspace registration", () => {
  ...
  expect(route).toContain('from "../session/ui/NewSessionDialog.js"')
