...
test("lazy detail UI has disclosure-triggered loading and accessible retry states", async () => {
  const row = await Bun.file(new URL("../../../src/session/ui/SessionSemanticStepRow.tsx", import.meta.url)).text()
  const history = await Bun.file(new URL("../../../src/session/ui/SessionSemanticHistory.tsx", import.meta.url)).text()
