...
test("the signed-in shell owns one feed, forwards status, and disposes it", async () => {
  ...
  expect(signedInApplication).toContain("fetch: fetcher")
  expect(signedInApplication).toContain("onAuthenticationError: options.sessionClear")
...
test("production HTTP states register every visible feed refresh seam", async () => {
  const sessionList = await source("src/session/ui/sessionListStateCreate.ts")
  const selectedSession = await source("src/session/ui/selectedSessionStateCreate.ts")
  ...
  expect(selectedSession).not.toContain("registerSelectedMessages")
  expect(selectedSession).toContain("registerSelectedDelegations")
  expect(selectedSession).not.toContain("registerSelectedStream")
  expect(selectedSession).toContain("sessionDetailStateCreate")
...
test("production session routes forward the signed-in fetcher into list and search state", async () => {
  ...
  expect(signedInApplication).toContain("eventFeedReconciliationCreate({ fetch: fetcher })")
...
test("production note routes forward the signed-in fetcher into list, create, and detail state", async () => {
  const notesRoute = await source("src/note/ui/NotesRoutePage.tsx")
  const newNoteRoute = await source("src/note/ui/NewNoteRoutePage.tsx")
  const noteRoute = await source("src/note/ui/NoteRoutePage.tsx")
  ...
  expect(notesState).toContain("noteListQueryStateCreate({")
  const noteListQuery = await source("src/note/ui/noteListQueryStateCreate.ts")
  expect(noteListQuery).toContain("noteListConditionalFetch({")
  expect(noteListQuery).toContain("fetch: options.fetcher")
