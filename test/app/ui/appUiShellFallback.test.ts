...
test("known application, demo, and simulation paths resolve to the UI shell", async () => {
  const paths = [
    ...
    "/sessions",
    "/sessions/recent/",
    "/sessions/pinned?session=selected",
    "/sessions/projects",
    "/sessions/search?search=term",
    "/sessions/selected?tab=pinned",
    ...
    "/demo/screens/session-generating",
...
test("known-route resolution rejects unknown paths and malformed parameter paths", () => {
  ...
  expect(appKnownRouteResolve("/sessions/unknown-id")).toBe(true)
  expect(appKnownRouteResolve("/sessions/a/b")).toBe(false)
