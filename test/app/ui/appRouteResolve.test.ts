...
test("settings is registered with the settings route page", async () => {
  ...
  const settingsSource = await Bun.file(
    new URL("../../../src/configuration/ui/SettingsRoutePage.tsx", import.meta.url),
  ).text()
  const settingsGeneralSource = await Bun.file(
    new URL("../../../src/configuration/ui/SettingsGeneralPanel.tsx", import.meta.url),
  ).text()
  const settingsSidebarSource = await Bun.file(
    new URL("../../../src/configuration/ui/SettingsSidebar.tsx", import.meta.url),
  ).text()
...
test("primary navigation reuses the shell-owned mobile session drawer", async () => {
  const navigationSource = await Bun.file(
    new URL("../../../src/ui/primaryNavigationStateCreate.ts", import.meta.url),
  ).text()
...
test("right-side header controls share the ghost utility-button treatment", async () => {
  ...
  const connectionSource = await Bun.file(
    new URL("../../../src/ui/ConnectionStatusIndicator.tsx", import.meta.url),
  ).text()
