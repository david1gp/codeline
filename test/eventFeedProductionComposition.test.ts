...
test("production session routes forward the signed-in fetcher into list and search state", async () => {
  ...
  expect(applicationRoot).toContain("authSessionStateCreate({ fetcher })")
  ...
  expect(signedInApplication).toContain("reconciliation: eventFeedReconciliationCreate({ fetch: fetcher })")
